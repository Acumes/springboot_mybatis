...
public class ImgValidateCode {
    ...
    private int[] defaultYPoints = { 15, 20, 10, 18, 22, 21 };
    private int[] xPoints = { 5, 15, 30, 50, 100, 130 };
    ...
    public void createCode() {
        ...
        int[] yPoints = shuffleArray(defaultYPoints);
        int nPoints = defaultYPoints.length;
    ...
    private int[] shuffleArray(int[] source) {
        int[] ret = Arrays.copyOf(source, source.length);
        Random random = new Random();
        random.nextInt();
        for (int i = 0; i < source.length; i++) {
            int change = i + random.nextInt(source.length - i);
            swap(ret, i, change);
        }
